...
def aggregate_attention_maps(attention_maps, num_heads, res=16):
    ...
    for m in attention_maps:
        if (m.shape[1] == res * res):
            m = rearrange(m, '(batch heads) hw seq -> batch heads hw seq',batch=1)
            m = torch.mean(m, dim=1)
            m = rearrange(m, 'batch (h w) seq ->batch h w seq',h=res)
            m = rearrange(m, 'batch h w seq ->batch seq h w')
            maps.append(m)
    stacked = torch.stack(maps)
    avg_map = stacked.mean(dim=0) 

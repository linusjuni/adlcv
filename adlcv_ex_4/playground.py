...
if __name__ == "__main__":
    ...
    example_images = np.stack(
        [im_normalize(tens2image(images[idx])) for idx in range(batch_size)], axis=0
    )
    ...
    plt.figure()
    plt.plot(
        range(1, diffusion.T + 1),
        diffusion.alphas.cpu().numpy(),
        label="alphas",
        linewidth=3,
    )
    plt.plot(
        range(1, diffusion.T + 1),
        diffusion.alphas_bar.cpu().numpy(),
        label="alphas_bar",
        linewidth=3,
    )
    plt.plot(
        range(1, diffusion.T + 1),
        diffusion.betas.cpu().numpy(),
        label="betas",
        linewidth=3,
    )
    ...
    noised_images = np.stack(
        [im_normalize(tens2image(xt[idx].cpu())) for idx in range(t.shape[0])], axis=0
    )
    show(
        noised_images,
        title="Forward process",
        fig_titles=fig_titles,
        save_path="assets/forward.png",
    )
    ...
    model.load_state_dict(
        torch.load(os.path.join(_here, "models", "weights-59epochs-full-dataset.pt"), map_location=device)
